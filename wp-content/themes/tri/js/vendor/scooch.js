(function (factory) {
    ...
    var Scooch = (function($, Utils) {
        ...
        Scooch.prototype._update = function() {
            ...
            //UiKit Update
            var viewPortWidth = this.$inner.prop('clientWidth'),
                $last = this.$items.eq(this._length -1),
                lastOffset = $last.prop('offsetLeft') + $last.prop('clientWidth'),
                maxLeft = lastOffset - viewPortWidth;

            if(currentOffset > maxLeft){
                this.$current = $last;
                this._index = this._length -1;
                x = Math.round(-maxLeft + this._offsetDrag);
            }
            //End UiKit Update
        ...
        Scooch.prototype.bind = function() {
            ...
            $element.on('click.Scooch', '[data-m-slide]', function(e){
                e.preventDefault();
        ...
        Scooch.prototype.destroy = function() {
            ...
            this.$inner.css(Utils.getProperty('Transform'), '');
            this.$element.off('click.Scooch');
